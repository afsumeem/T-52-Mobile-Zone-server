...
async function run() {
  try {
    ...
    app.get("/products/:id", async (req, res) => {
      const productDetail = await productCollection.findOne({
        _id: req.params.id,
      });
    ...
    app.put("/users", async (req, res) => {
      ...
      const result = await saveUsersCollection.updateOne(
        filter,
        updateDoc,
        options
      );
    ...
    app.delete("/users/:id", async (req, res) => {
      const deleteUser = await saveUsersCollection.deleteOne({
        _id: ObjectId(req.params.id),
      });
      res.json(deleteUser);
    });
